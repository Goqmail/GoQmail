...
class Email(db.Model):
    ...
    body = Column(Text)
    is_html = Column(Boolean, default=False)  # Flag to indicate if content is HTML
    ...
    def to_dict(self):
        ...
        result = {
            ...
            'from': self.sender or '',
            'sender': self.sender or '',  # Include both sender and from for template compatibility
            'date': self.date or '',
            'body': self.body or '',
            'is_html': self.is_html,  # Include HTML flag
            'error': self.error,
            'message': self.error_message if self.error else None,
            # Include default security_analysis field for templates
            'security_analysis': {
                'risk_level': 'Cautious',
                'security_score': 5,
                'suspicious_patterns': [],
                ...
                'domain': extract_domain_from_email(self.sender) if self.sender else ''
            }
        }
        # Print a debug message to track what we're returning
        import logging
        logger = logging.getLogger(__name__)
        logger.debug(f"Converting email {self.id} to dict: {result}")
        return result
